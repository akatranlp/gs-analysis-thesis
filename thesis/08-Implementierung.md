...
Aus der Punkten der Recherche und dem wird nun ein Prototyp erstellt. Der Prototyp besteht aus mehreren Teilen, die zum Teil miteinander verbunden sind oder getrennt voneinander betrachtet werden können.
...
![App Loop Funktion](./images/app-loop.png){ height=1200px }
...
Die Implementation wurde mit dem Package "PromiseSocket" ^[https://www.npmjs.com/package/promise-socket] getätigt. Dieses Package wandelt das Eventdriven-system von der Node-Socket API in eine Promise-Based-API um. Somit werden keine EventListener mehr von außen registertiert, sondern entweder mit then oder await gearbeitet wie besispielsweise:

```typescript
const response = await promiseSocket.read();
```

Wie bereits im [Abschnitt 5.5.2](#struktur-der-packete) angesprochen muss man bevor man Rcon-Commands senden kann authentifizieren.
...
Hierbei kam es aber wieder zu Problemenen durch unterschiedliche Implementationen des Rcon-Servers. Bei Conan Exiles sind die IDs der Antworten immer um eine Anwort verschoben. ([siehe Abbildung 21](#rcon-bug))
...
![Conan Exiles Rcon Bug](./images/conan-rcon-bug.png){ #rcon-bug width=1500px }
...
Ein Problem bestand zu diesem Zeitpunkt noch. Die Gameserver konnten zwar über Docker gestartet werden aber wie wird der Hostserver gestartet. Hierbei verwendet wir das sogennante Magic-Paket. Dabei handelt es sich um ein UDP-Datenpaket welches über Broadcast an alle Geräte im Subnetz verschickt wird und die MAC-Adresse des zu startenden PCs beinhaltet. Ist die Netzwerkkarte Magic-Paket fähig und sind die benötigten Einstellungen im BIOS und im Betriebssystem getroffen worden, startet der PC beim Erhalt eines solchen Pakets. In der folgenden Abbildung finden wir den Source-Code zum Versenden dieses Magic-Packets.

![Source Code senden eines Magic-Packets](./images/magic-packet.png){ width=1300px }