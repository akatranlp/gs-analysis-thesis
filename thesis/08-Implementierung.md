...
Die Implementation wurde mit dem Package "PromiseSocket"^[https://www.npmjs.com/package/promise-socket] getätigt. Dieses Package wandelt das Eventbasierte-System von der Node-Socket API in eine Promise-Based-API um. Somit werden keine EventListener mehr von außen registriert, sondern entweder mit "then" oder "await" gearbeitet wie beispielsweise:
...
Alternativ gibt es die Funktion "sendCommandWithLongResponse". Diese Funktion wird in dem Prototyp so nicht genutzt, bietet aber die Möglichkeit nach dem oben genannten Schema mit 2 Command-Anfragen auf eigene Gefahr auf die Antwort der Nachricht mit der zweiten ID zu warten.
