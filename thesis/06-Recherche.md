...
In den folgenden Abschnitten werden verschiedene Methoden ermitteln wie die Anzahl der Spieler auf einem Server ermittelt werden kann.
...
Das Transmission Control Protocol (TCP) ist eines der Haupt-Protokoll des Internets und wird zusätzlich zum Internet-Protokoll (IP) verwendet, um um eine zuverlässige Übertragung von Paketen zu gewährleisten. 

Ein TCP Paket besteht aus vielen Feldern im Header, die alle benötgt werden, um eine zuverlässige Datenübertragung zu gewährleisten, siehe [Abbildung 5](#tcp-paket).

Dazu gehören unter anderem Quell-Port und Ziel-Port, Sequenz- und Bestätigunngsnummer und eine Prüfsumme sowie einige Flags die den Fluss der Daten managen. [@gudabayev_tcp_2021]
...
Im Gegensatz zu TCP ist das User Datagram Protocol (UDP) ein verbindungsloses Protkol. Es wird somit keine Verbindung zwischen Client und Server aufgebaut, sondern Pakete werden einfach von a nach b geschickt und gehofft das es ohne Fehler beim Empfänger ankommt. [@gudabayev_udp_2021]

UDP wird auch zusätzlich zum Internet-Protokoll (IP) eingesetzt und ist das zweite Haupt-Protokoll des Internets.

Der Header eines UDP Pakets ist ziemlich simpel und besteht aus dem Quell-Port, Ziel-Port, Länge des Headers und der Daten und einer Prüfsumme, siehe [Abbildung 7](#udp-paket).
...
Bei UDP sieht dies anders aus. Da es ein verbindungsloses Protokoll ist gibt es in UDP kein Konzept der Verbindung und somit keine kontinuierliche Verbindung zwischen Client und Server. Da aber ein großteil der Gameserver nur über das UDP Daten vom Client erhalten und an den Client senden müssen wir hier die Verbindung anders definieren. [@fiedler_client_2016]
...
OpenSense ^[https://docs.opnsense.org/manual/diagnostics_firewall.html] und PfSense ^[https://docs.netgate.com/pfsense/en/latest/monitoring/status/firewall-states-gui.html] bieten hierfür in den Diagnostics der Firewall mit denen man den Status der aktiven Verbindungen betrachten kann. OpenSense bietet hierfür eine API mit dem man diese Werte auch außerhalb der Weboberfläche erhalten kann. PfSense hat nur mit Hilfe des Packages pfsense-api^[https://github.com/jaredhendrickson13/pfsense-api] eine API mit dem man dann auch die Daten erhalten kann.
...
Wenn kein Router mit einer API vorhanden ist an dem die aktiven Verbidnungen auslesen kann. Müssen die Verbindungen an einer anderen Stelle analysiert werden.

Eine Möglichkeit ist mit Hilfe einem TCP-Forwarder, da ich keine Artikel zu diesem Thema gefunden habe, habe ich ein eigenes "Proof of Concept" erstellt.

In node wurden die Pakete "dgram" ^[https://nodejs.org/api/dgram.html] und "net" ^[https://nodejs.org/api/net.html] genutzt um jeweils für die beiden Protokolle udp und tcp einen Forwarder zu erstellen.
...
Man kann die Anzahl der Verbindungen am Server selbst messen mit verschiedenen Tools wie hier angesprochen tcpdump ^[https://www.tcpdump.org/manpages/tcpdump.1.html]

Mit tcpdump kann man den Netzwerk Verkehr des Servers mitschneiden und analysieren, zudem kann man es auf bestimmte Interfaces und bestimmte Ports beschränken, sodass nicht alle Pakete die dem Server geschickt oder vom Server versendet werden in den Ergebnis auftauchen.
...
RCON ist ein TCP/IP basiertes Kommunikationsprotokoll, welches es erlaubt Konsolen Befehle auf dem Server auszuführen mit einer Fernzugriff-Konsole ("Remote Console").
[@valve_source_rcon_protocol_nodate]

Rcon wird inzwischen von mehreren Gameservern unterstützt um die Server einfacher zu administrieren. Alle Valve GameServer wie Counter Strike und Team Fortress 2 ^[https://developer.valvesoftware.com/wiki/Source_RCON_Protocol] sowie weitere Spiele von anderen Herstellern wie Minecraft ^[https://wiki.vg/RCON] und Conan Exiles ^[https://conanexiles.fandom.com/wiki/Rcon].

### Struktur der Packete

In der folgenden [Abbildung](#packet-structure) ist die Struktur eines RCON-Packets abgebildet. Es beinhaltet die Größe des gesamten Packets, eine ID die frei vom Client gewählt werden kann und in der Antwort vom Server zurückgeschickt wird, einen Typ auf den gleich näher eingegangen wird, einen Body mit dem Inhalt des Packets und ein null-Byte am Ende um das Packet abzuschließen. [@valve_source_rcon_protocol_nodate]

![Rcon Packet Struktur (https://developer.valvesoftware.com)](./images/rcon-basic-paket.png){ #packet-structure }

Es gibt insgesamt 4 verschiedene Typen in der RCON-Spezifikation. Wobei SERVERDATA_AUTH_RESPONSE und SERVERDATA_EXECCOMMAND den selben Wert besitzen. Da 
...
Dabei handelt es sich um die Antwort auf das Serverdata-auth Packet. Die ID dieses Packets ist -1 wenn das mitgesendete Passwort falsch ist und entspricht der ID der Anfrage wenn es richtig ist. 

- SERVERDATA_EXECCOMMAND
Mihilfe dieses Packets übermittelt der Client seinen auszuführenden Befehl. 
...
Minecraft hat den Befehl "list", Conan Exiles den Befehl "listplayers" und TF2 den Befehl "users". Der Output dieser Befehle ist auch nicht vereinheitlicht, weshalb diese jeweils anders analysiert werden muss, um in einem Skript die genaue Anzahl der Spieler zu erhalten.
